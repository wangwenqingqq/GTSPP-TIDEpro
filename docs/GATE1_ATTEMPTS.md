...
8. `campaign_v4_gpu0`：同样 21 个子进程全部通过，325.05 秒 wrapper 墙钟，
   983,040 个计时逻辑查询完整匹配，观测显存均在两档预算内。all-delta
   shadow/static 的 Q=1 p99 比值从 v3 的 17.57 回落到 v4 的 1.09；后者
   四进程区间含 1，服务延迟比仍约 1.024。92,320 个固定可见集合 batch
   和 1,248 个维护事件的语义/字节量前后相同。完整 v3/v4 分别保留；
   结论和曝光限制见 [Gate 1 结果](GATE1_RESULTS_20260913.md)。
