...
## 普通 staging 复用对照（v3 完整采集后、v4 之前）

v3 固定可见集合的尾部干扰明显，但维护总时间中仍有未细分的普通 host/API
开销。v4 只将每 run 的 8 MiB pinned staging 注册/注销，改为 Writer 生命周期
中一次注册并复用；各 run 仍按相同字节/块进行 host memcpy 和 H2D。
所有其它预设不变，2,688 个 oracle 重生成对照，21 子进程门禁/性能协议
全部重跑。v3 保留为有效前版证据，v4 作为复用 staging 的普通控制。
前后顺序固定，并非随机交错；改善只能作为支持该解释的对照，不夸大因果。
